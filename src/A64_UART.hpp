...
struct A64_UART {
    ...
    void initalize() volatile {
        auto portController = A64_PortController::instances[0].address;
        portController->banks[1].configure[1] |= (8<<4)|8; // UART0 TX(8), RX(9)
        portController->banks[1].pull[0] |= 1<<(9*2); // UART0 RX(9)

        auto CCU = A64_CCU::instances[0].address;
        CCU->BusClock_Gating3 |= 1<<16; // UART0
        CCU->Bus_Software_Reset4 |= 1<<16; // UART0

        lineControl.divisorLatchAccessBit = 1;
        divisorLatchLow = 13; // 115200 baud
        lineControl.divisorLatchAccessBit = 0;
        lineControl.dataLengthSelect = 3;
    }
};

const struct A64_UART::Instance A64_UART::instances[] = {
    { reinterpret_cast<A64_UART*>(0x01C28000), 32, 6 },
    { reinterpret_cast<A64_UART*>(0x01C28400), 33, 7 },
    { reinterpret_cast<A64_UART*>(0x01C28800), 34, 8 },
    { reinterpret_cast<A64_UART*>(0x01C28C00), 35, 9 },
    { reinterpret_cast<A64_UART*>(0x01C29000), 36, 10 }
};
